...
export default function Users() {
  ...
  const [editForm, setEditForm] = useState({ name: '', role: 'staff' as 'admin' | 'staff', status: 'Active' as 'Active' | 'Inactive' });
  ...
  const createStaff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await logAction('User Created', newStaff, `Created ${form.role} account for ${form.name} (${form.email})`);
      ...
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4 text-center">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1 max-w-sm">
            Staff management is only accessible to Admin users.
          </p>
        </div>
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredStaff.map(s => (
                    <tr key={s.user_id} className="hover:bg-slate-50/60 transition">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                            s.role === 'admin' ? 'bg-teal-100 text-teal-700' : 'bg-blue-100 text-blue-700'
                          }`}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-lg font-semibold border ${ROLE_COLORS[s.role]}`}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`text-xs px-2 py-0.5 rounded-lg font-medium ${ROLE_COLORS[s.role]}`}>
                            {s.role}
                          </span>
              ...
              <div className="flex items-start gap-2 p-3 bg-amber-50 rounded-xl border border-amber-100">
                ...
                <p className="text-xs text-amber-700">
                  {form.role === 'admin'
                    ...
                    : 'Staff users can manage patients, appointments, and treatments but cannot access billing or staff management.'}
                </p>
              </div>
              <button
                type="submit"
